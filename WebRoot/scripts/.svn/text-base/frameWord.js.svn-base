...
function addNewTabWelcome(obj,url,title){
	$(obj).tabs('add',{
		id:1,
		title:title ,
		closable:false,
...
function welcomeTab(url,title,over){
	var tab_center=document.getElementById("tab_center");	
	...
	url=url+"&datetval="+new Date().getTime();
	if(over=="true"){
		if($(tab_center).tabs("exists",title)){
			selectOverTab(tab_center,url,title);
			addNewTabWelcome(tab_center,url,title);
		} else if(tabCount<10){
			addNewTabWelcome(tab_center,url,title);
		}else if(tabCount>=10){
			alert("最多只能有10个窗口");
